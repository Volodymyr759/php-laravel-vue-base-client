<template>
  <div class="filters-wrapper">
    <a-input-search v-model:value="search" placeholder="Search" style="width: 308px" size="large" @search="onChangeSearchText" />
  ...
  <div class="table-wrapper">
    <a-row justify="center">
      <a-col>
        <lease-table :settings="leaseService.pageSettings" @edit-lease="(lease) => onEditLease(lease)"
          @delete-lease="(id) => onDeleteLease(id)" @change-page="(current) => onChangePage(current)" />
        <a-modal v-model:visible="visible" :title="currentLease && currentLease.id ? 'Update Lease' : 'Create Lease'"
          :footer="null" :destroyOnClose="true">
          <lease-form :lease="currentLease ? currentLease : undefined"
            @submit-leaseform="(lease) => onSubmitLease(lease)" @close-leaseform="visible = false" />
        </a-modal>
      </a-col>
    </a-row>
  </div>
...
export default defineComponent({
  ...
  setup() {
    ...
    return {
      leaseService, currentLease,
      search, onChangeSearchText, onChangeLeaseStatus,
      defaultLeaseStatus: ref("All"), LeaseStatus,
      visible,
      onSubmitLease, onCreateLease, onEditLease, onDeleteLease, onChangePage
    };
  },
});
</script>