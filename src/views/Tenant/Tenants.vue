<template>
  <div class="filters-wrapper">
    <a-input-search v-model:value="search" placeholder="Search" size="large" style="width: 308px" @search="onChangeSearchText" />
    <a-select size="large" @change="onChangeSort" v-model:value="defaultSort">
  ...
  setup() {
    ...
    function onChangeSearchText(searchValue: string): void {
      searchValue.trim().length === 0
        ? (tenantService.pageSettings.filters[0].value = "")
        : (tenantService.pageSettings.filters[0].value = searchValue);
    ...
    function onChangeSort(sortValue: string): void {
      switch (sortValue) {
        case "id_asc":
          tenantService.pageSettings.sorting = [
